...
echo -e "${GREEN}=== Constructor de APK para YouTube Downloader ===${NC}"
...
# Usamos --overwrite para asegurar que se actualice con los últimos cambios de nuestro código Python
echo "Generando configuración nativa..."
flet create . --project "YoutubeDownloader" --description "Descargador de videos offline"

# 3. Compilación
echo -e "${YELLOW}[3/4] Compilando APK (Esto puede tardar unos minutos)...${NC}"
echo "Ejecutando: flet build apk"

# Ejecutamos el build. Nota: flet build apk usa la configuración generada en el paso anterior.
yes | flet build apk
...
DEST_APK="./YouTubeDownloader.apk"
...
if [ -n "$SOURCE_APK" ] && [ -f "$SOURCE_APK" ]; then
    ...
    echo "2. Ejecuta: adb install YouTubeDownloader.apk"
else
